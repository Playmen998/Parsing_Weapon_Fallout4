...
Переменные
"""
type_weapon = [] # Тип оружия - список
[type_weapon.append(type.text) for type in soup.find_all('li', class_ = "wds-tabs__tab")[:-1]]

all_table = [] # сохранение списка со всей информацией

"""
Поиск параметров для оружия (бомб)
"""
items = web_page[6].find_all("tr")
k = 1
while k != len(items):
    ...
    line_weapon.pop(0)
    line_weapon.insert(1, type_weapon[6])
    line_weapon.pop()
    all_table.append(line_weapon)
    k += 1

df = pd.DataFrame(data =all_table,columns = ["Название",'Тип',"Тип_Оружия","Урон", "Вес", "Цена"])
df.drop('Тип', axis = 1)
df.to_csv('bomb.csv', index = True)